...
ModelDriven<Admin>,Preparable,ServletResponseAware{
	...
	public String CheckSystemAdmin(){
		System.out.println("CheckSystemAdmin");
		String idno=model.getIdno();
		String passwordMd5=MD5Utils.getHash(model.getPassword(), "MD5");
		boolean flag=adminService.checkSystemAdmin(idno,passwordMd5);
		if(flag){
			session.put("systemAdminIdno", idno);
			return "checkSystemAdmin-success";
		}
		else return "checkSystemAdmin-fail";		
	}
	public String showSystemAdmin(){
		String idno=(String) session.get("systemAdminIdno");
		System.out.println(idno);
		Admin systemAdmin=adminService.showSystemAdmin(idno);
		if(systemAdmin!=null){
				request.put("Info", systemAdmin);
				return SUCCESS;
		}
		else{
			return ERROR;
	...
	public String UpdateSystemAdmin(){
		String idno=(String) session.get("systemAdminIdno");
		boolean flag=adminService.updateSystem(idno,model);
		if(flag){
			showSystemAdmin();
			return SUCCESS;
		}
	...
	public void prepareCheckIdno(){
		System.out.println("prepareCheckIdno");
		model =new Admin();
	}
	...
	public void prepareCheckSystemAdmin(){
		System.out.println("prepareCheckSystemAdmin");
		model =new Admin();
	}
	public void prepareSave(){
		model =new Admin();
	}
	public void prepareUpdateSystemAdmin(){
		model =new Admin();
	}
